...
printInteraction <- function(ph){
  txt = "Posthoc tests for each layer"
  lastest=NULL
  for (mi in 1:length(ph)){
    ...
    mtest = m[[1]]$test
#    print(m[[1]]$test)
    pval = round(mtest$pvalues,4)
    est = abs(round(mtest$coefficients[[1]],4))

    if (pval < 0.05){
        tstat = abs(round(mtest$tstat[[1]],2))
        df = m[[1]]$df
        pvalstr = format.pval(pv = pval, digits = 2,eps=0.001,nsmall = 3)
        if (substr(pvalstr,1,1)!='<'){
           pvalstr = paste("=",pval,sep="")
        }
        wordstr = paste("@@Posthoc: There was a significant difference for ",mname,", diff = ",est,", t(",df,")=",tstat,", p",pvalstr,sep="")
    }else{
         wordstr = paste("@@Posthoc: There was no difference for ",mname,", p=",pval,sep="")
    }
    txt = paste(txt,"\n",wordstr)
    if (!is.null(lastest)){
      estlast = est/lastest
      lastest = lastest/est
      txt = paste(txt,"\nratio ",mname," ",estlast," rev ",lastest)
    }
    lastest = est
  }
  return(txt)
}
#printInteraction(posthocs)
...
printMixedModelResults<- function(modellist){
...
#      pred = model@frame[,col]
      meandf = aggregate(as.formula(paste(dependMeasure," ~ ",paste(col,collapse=" + "),sep="")), model@frame,mean)
      meandf[,dependMeasure]=round(meandf[,dependMeasure],4)
...
## Rommer Federmeier Not seen

```{r,fig.height=4,fig.width=12}
write("Rommer Federmeier Not Seen", stderr())
...
p6word$Lexical = c("-ss","-ed", "the","a")
#p = c(0.47,0.47,0.03,0.03)
p = c(0.3,0.3,0.1,0.1)
p6word$Predicted = p
p6word$Target = 0 
p6word$Target[p6word$Lexical == "-ss" & p6word$Grammaticality == "Gram."] = 1
p6word$Target[p6word$Lexical == "the" & p6word$Grammaticality == "Ungram."] = 1
...
dfall=dfall[dfall$variable!="absError",]
dfall$Lexical=factor(dfall$Lexical,levels=c("a","the","-ed","-ss"))
#dfall$Shape = dfall$Lexical
dfall
dfall$endlen= 0.2
dfall$nu=0
...
p = p + geom_hline(yintercept = 0,linetype="dashed",size=0.5)
p = p + scale_x_continuous(breaks = 0:4,labels=c("", "a","the","-ed","-ss")) 
p = p + theme_bw()+theme(legend.position="bottom")
p=p + theme(panel.grid.major = element_blank(), 
                  panel.grid.minor = element_blank(),
                  panel.background = element_blank(), panel.spacing = unit(0.5, "lines"),
...
p6word4$Lexical = factor(p6word4$Lexical,levels=c("a","the","-ed","-ss"))
...
p = p + geom_hline(yintercept = 0,linetype="dashed",size=0.5)
p = p + scale_x_continuous(breaks = 0:8,labels=c("", "a","the","-ed","-ss", "a","the","-ed","-ss")) 
